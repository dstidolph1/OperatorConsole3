          ...
          if (line.length()) {
              ...
              if ((pLeft = line.find_first_of(";#[=")) != string::npos) {
                  ...
                  case '=':
                      ...
                      // strip comment
                      if (value.size() && value.find_first_of(";#") != string::npos)
...
bool CIniFile::SetValueV( string const keyname, string const valuename, char *format, ...)
{
  ...
  vsprintf_s( value, sizeof(value), format, args);
  va_end( args);
...
CPoint CIniFile::GetValuePT(string const keyname, string const valuename, CPoint const defValue) const
{
    long keyID = FindKey(keyname);
    if (keyID == noID)
        return defValue;
    ...
    CPoint pt;
    char szDefValue[MAX_VALUEDATA];
    sprintf_s(szDefValue, sizeof(szDefValue), "%d,%d", defValue.x, defValue.y);
    std::string sValue = GetValue(keyname, valuename, szDefValue);
    size_t commaPos = sValue.find(',');
    if (commaPos != std::string::npos)
    {
        std::string sXValue = sValue.substr(0, commaPos); // from start to before the comma
        std::string sYValue = sValue.substr(commaPos + 1); // after the comma
        pt.x = atoi(sXValue.c_str());
        pt.y = atoi(sYValue.c_str());
    }
    return pt;
}
